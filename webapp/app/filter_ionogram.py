...
import pandas as pd
import ipdb
from .update_tx_code import create_db_connection, update_virginia_tx_code
...
def filter_ionograms(dirs1, data, f, DataDict, normalize_by_frequency=True):
    ...
    pos1 = n.argwhere((arr1 > int(data['chriprate'])) & (arr1 < data['first_hop_range_one'])) # filters are applied here.
    #ipdb.set_trace()                                           
    ch1 = DataDict['ch1']
    ...
    if ((Rate == int(data['chriprate'])) and (data['first_hop_range_one']< r0 < data['first_hop_range_two'])) |((Rate == int(data['chriprate'])) and
       (data['second_hop_range_one'] < r0 < data['second_hop_range_two']) and (len(pos1) > 0)):
        ...
        update_virginia_tx_code(connection, f)
        print('ch1_inside=%d' %(ch1))
    return DataDict
